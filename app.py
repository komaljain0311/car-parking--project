...
# Prepare directory for saved parking slots
...
# Load parking slots if available
...
if cap:
    ...
    while cap.isOpened():
        ...
        for i, polygon in enumerate(slot_list):
            ...
            cv2.putText(frame, f"{i+1}: {status}", tuple(polygon[0]), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
